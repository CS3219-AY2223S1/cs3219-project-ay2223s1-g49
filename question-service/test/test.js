...
describe("Test create questions", () => {
    ...
    it("Should be blocked", done => {
        ...
        const VALID_ANSWER = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
        const VALID_PAYLOAD = { difficulty: VALID_DIFFICULTY, content: VALID_CONTENT, answer: VALID_ANSWER }
        chai.request(app)
        .post(URL)
        .send(VALID_PAYLOAD)
        .end( (req,res) => {
...
describe("Test questions limit", () => {
    ...
    before(async () => {
        ...
        const hardQuestion = QuestionModel({ difficulty: "hard", content: "  cool  ", answer: "this is answer" })
        await hardQuestion.save()
        const easyQuestion1 = QuestionModel({ difficulty: "easy", content: "  cool1  ", answer: "this is answer" })
        await easyQuestion1.save()
        const easyQuestion2 = QuestionModel({ difficulty: "easy", content: "  cool2  ", answer: "this is answer" })
...
describe("Test quesions random id", () => {
    ...
    before(async () => {
        ...
        hardQuestionIdSet = (await QuestionModel.find({difficulty: "hard"})).map(doc => doc.id)
        easyQuestionIdSet = (await QuestionModel.find({difficulty: "easy"})).map(doc => doc.id)
    })
...
describe("Test quesions random id", () => {
    ...
    before(async () => {
        // Clear DB and insert test user
        await QuestionModel.deleteMany({})
        const hardQuestion = QuestionModel({ difficulty: "hard", content: questionContent, answer: "this is answer" })
        await hardQuestion.save()

        questionID = (await QuestionModel.find({difficulty: "hard"})).map(doc => doc.id)[0]
    })
...
describe("Test quesions random id", () => {
    ...
    before(async () => {
        // Clear DB and insert test user
        await QuestionModel.deleteMany({})
        const hardQuestion = QuestionModel({ difficulty: "hard", content: questionContent, answer: questionAnswer })
        await hardQuestion.save()
